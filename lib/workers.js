    ...
    if(!err && checks.length > 0){
        ...
        _data.read("checks", check, (err, initialCheck) => {
            if(!err && initialCheck){
                ...
                //debug("Error Couldn't read one of the checks");
            }
        });
        });
        ...
        //debug("Error: Couldn't find any check to process");
    }
...
workers.validateCheck = function(initialCheck){
        ...
        if(protocol && url && method && successCodes && timeout && phone && id){
            ...
            ////debug("Erro:Couldn't validate one of the initial check fields/property. Skipping it");
        }
};
...
workers.processCheckOutcome = (updatedCheck, checkOutCome) => {
    ...
    //debug("Check outcome has not changed, no alert needed!");
        ...
        //debug("Error:Couldn't update one of the checks");
    }
 });
     ...
     _helpers.twilioSms(phone, msg, (err) => {
        if(!err){
           //debug("Success: ", msg);
            ...
            //debug("Error:Couldn't send alert!");
        }
    });
...
workers.log = function(initialCheck, checkOutCome, state, canAlert, checkTime){
    ...
    _logs.append(logFileName, logDataToString, (err) => {
        if(!err){
            //debug(`Logging to ${logFileName} file Succeeded!`);
        } else {
            //debug(`Logging to ${logFileName} failed!`);
        }
    });
            ...
            _logs.compresseLogs(log, `${log}-${Date.now()}`, (err) => {
                if(!err){
                    _logs.truncate(log, (err) => {
                      if(!err) {
                        //debug(`Success:log with log id ${log} was compressed and the original log truncated and ready to receive new logs for the day`)
                      } else {
                        //debug(err);
                      }
                    });
