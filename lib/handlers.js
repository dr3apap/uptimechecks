...
//TODO:seprate dellAll and deleByID into there seprate handlers, Have a hanldler for changing usersphone and make it propagate to all resources holding by the user.Make sure user can't access any resources if token is expire the need to request fresh token
...
handlers._users = {
    ...
    get(data, callback) {
        ...
        if (phone) {
            console.log(phone);
            const tokenId = typeof (data.headers.token) == "string" && data.headers.token.trim().length == 20 ? data.headers.token : false;
            console.log("User info before validation", tokenId);
            handlers._tokens.validateUser(tokenId, phone, (tokenIsValid) => {
                console.log("Validating user info", tokenId, phone);
                if (tokenIsValid) {
                    _data.read("users", phone, (err, data) => {
                        if (!err && data) {
...
handlers._checks = {
    ...
    get(data, callback) {
        ...
        const allChecksPath = data.trimmedPath;
        if (checkId) {
            getCheckById(data, callback);
        } else {
            if (allChecksPath.includes("all")) {
            getAllChecks(data, callback);

            }
        }

        function getAllChecks(data, callback) {
            const tokenId = typeof (data.headers.token) == "string" && data.headers.token.trim().length == 20 ? data.headers.token : false;
            if (tokenId) {
                _data.read("tokens", tokenId, (err, tokenData) => {
                    if (!err && tokenData) {
    ...
    delete(data, callback) {
        ...
        checksId ? deleteById(data, callback) : deleteAllChecks(data, callback);
        ...
        function deleteAllChecks(data, callback) {
            //let checkId = data.queryStringObject.id;
            let tokenId = data.headers.token;
            //checksId = typeof(checksId) == "string" && checksId.trim().length == 20?checksId:false;
            tokenId = typeof (tokenId) == "string" && tokenId.trim().length == 20 ? tokenId : false;
